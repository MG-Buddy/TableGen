...
default_config = {
    "origenes": {
        ...
        "GP-Chihuahua": "T CHI",
        "GP-Cadereyta": "T CAD",
        "BLK-San Luis Potosi": "TCM-SLP",
        "BLK-Salinas Victoria": "BLK-SVIC",
        "MA-Veracruz": "T VER",
        "MA-Tula": "T TUL",
        "MA-Puebla": "T PUE",
        "RP-MAD": "RP-MAD",
...
if not os.path.exists(config_path):
    for key in ["HideR", "HideP", "HideD", "IgnoreOrigen"]:
        default_config[key] = {origen: False for origen in default_config["origenes"]}
    with open(config_path, 'w', encoding="utf-8") as f:
        json.dump(default_config, f, indent=4, ensure_ascii=False)
    print("Archivo de configuración creado con valores por defecto.")

# Carga de elementos MacOS
...
# Verifica que se use lastest
def check_updates():
    url = "https://raw.githubusercontent.com/MG-Buddy/TableGen/refs/heads/main/default/remote_data.json"
    try:
        ...
        if remote_version and remote_version != __VERSION__:
            msg = f"Tu versión actual es {__VERSION__}.\nLa versión más reciente es {remote_version}.\n\n¿Deseas el link con la version nueva?"
            if messagebox.askyesno("Actualización disponible", msg):
                if download_url:
                    webbrowser.open(download_url)
                else:
                    messagebox.showinfo("Info", "No se encontró un enlace de descarga.")
        else:
...
def StString(str):
    if str in config['origenes']:
        ...
        messagebox.showwarning("Advertencia", f"No se puede estandarizar {str} a un origen. No existe en el archivo de Alias'es.\n\nSe imprimirá tal cual.")
        return str
...
def GenTable(cliente, origenes, destinos, precios, formato, fecha, IgnoreOrigen, HideR, HideP, HideD, isDetailed, cf=None, iva=None, flete=None, base=None, extra=None, ut=None, benefs=None, typeut=None, notas=None):
    try:
        ### START-Base ###
        print(f"Printing {cliente} in format {formato} - Ignoring {IgnoreOrigen} [{HideR} - {HideP} - {HideD}]")
        ...
        truck_img = Image.open(resource_path("assets/LilTruck.png")).convert("RGBA")
        ...
        for row_idx, destino in enumerate(destinos):
            ...
            for key in productos_validos:
                ...
                ### START-PricesDETAILED ###
                    ...
                    if isDetailed == 1:
                        if "GAS 87" in key and origen in HideR:
                            v_price = "     -"
                        elif "GAS 91" in key and origen in HideP:
                            v_price = "     -"
                        elif "DIESEL" in key and origen in HideD:
                            v_price = "     -"
                        else:
                            v_price = f"${productos_validos[key][row_idx]:.2f}" if not np.isnan(productos_validos[key][row_idx]) else "     -"
...
check_updates()
root = tk.Tk()
root.title(f"Generador de Tablas v{__VERSION__}")
...
precios_check = tk.Checkbutton(root, text="Precios Desglosados", variable=precios_desglosados_var, onvalue=True, offvalue=False)
precios_check.pack()
